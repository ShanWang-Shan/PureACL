...
gps_ref_lat = 42.294319*np.pi/180.0
gps_ref_long = -83.223275*np.pi/180.0
gps_ref_height = 0.0
...
def angular_distance_to_xy_distance( lat, long):
    # dx = ru_t * np.cos(gps_ref_lat) * (long1 - gps_ref_long)
    # dy = ru_m * (lat1 - gps_ref_lat)

    gps_lat = lat * np.pi / 180.0
    gps_long = long * np.pi / 180.0

    # ru_m_local = semi_a * (1.0 - eccentricity_square) / np.power(
    #     1.0 - eccentricity_square * np.sin(gps_lat) * np.sin(gps_lat), 1.5)
    #
    # ru_t_local = semi_a / np.sqrt(1.0 - eccentricity_square * np.sin(gps_lat) * np.sin(gps_lat))
    # dx = ru_t_local * np.cos(gps_ref_lat) * (gps_long - gps_ref_long)
    # dy = ru_m_local * (gps_lat - gps_ref_lat)

    dx = ru_t * np.cos(gps_ref_lat) * (gps_long - gps_ref_long)
    dy = ru_m * (gps_lat - gps_ref_lat)
    return dx, dy
